...
public interface INorthWindSalesCommandsDataContext
{
  ...
  Task<Entities.Product> FindProductByIdAsync(int productId);
  void UpdateProduct(Entities.Product product);

  // Nuevo: encontrar cliente por id
  Task<Entities.Customer> FindCustomerByIdAsync(string customerId);
}

//  Interfaz adicional para operaciones administrativas (CRUD) sobre entidades
//  como Customer y Product desde la API.
public interface INorthWindSalesAdminDataContext
{
    ...
    // Nuevos métodos para validar integridad referencial
