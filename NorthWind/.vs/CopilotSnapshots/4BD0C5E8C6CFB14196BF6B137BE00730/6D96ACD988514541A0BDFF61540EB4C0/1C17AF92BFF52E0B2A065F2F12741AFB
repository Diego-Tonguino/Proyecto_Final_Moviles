...
ICommandsRepository
{
    ...
    {
        // Verificar existencia de cliente antes de crear la orden (para evitar race condition)
        var customer = await context.FindCustomerByIdAsync(order.CustomerId);
        if (customer == null)
        {
            throw new CustomerNotFoundException($"El cliente {order.CustomerId} fue eliminado. Por favor, intenta crear la orden nuevamente.")
            {
                CustomerId = order.CustomerId
            };
        }

        // Validate stock and decrement product units
        foreach (var detail in order.OrderDetails)
        {
            var product = await context.FindProductByIdAsync(detail.ProductId);
            if (product == null)
            {
                // Lanzar excepción personalizada cuando el producto no existe
                // (fue eliminado entre la validación inicial y el guardado - race condition)
                throw new ProductNotFoundException(
                    $"El producto {detail.ProductId} fue eliminado. Por favor, intenta crear la orden nuevamente.")
                {
            ...
            {
                // Custom message for insufficient stock
                throw new System.InvalidOperationException($"Insufficient stock for product '{product.Name}'. Requested: {detail.Quantity}, Available: {product.UnitsInStock}.");
            }
