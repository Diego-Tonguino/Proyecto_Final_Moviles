...
internal class NorthWindSalesCommandsDataContext(IOptions<DBOptions> dbOptions)
    : NorthWindSalesContext(dbOptions), INorthWindSalesCommandsDataContext, INorthWindSalesAdminDataContext
{
  ...
  public async Task AddCustomerAsync(Repositories.Entities.Customer customer) => await AddAsync(customer);
  public async Task UpdateCustomerAsync(Repositories.Entities.Customer customer) => Update(customer);
  public async Task DeleteCustomerAsync(string customerId)
  {
    var entity = await Customers.FindAsync(customerId);
    if (entity != null) Remove(entity);
  }

  // Admin CRUD for Products
  public async Task AddProductAsync(Repositories.Entities.Product product) => await AddAsync(product);
  public async Task UpdateProductAsync(Repositories.Entities.Product product) => Update(product);
  public async Task DeleteProductAsync(int productId)
  {
    var entity = await Products.FindAsync(productId);
    if (entity != null) Remove(entity);
  }

  // Métodos para gestionar productos (validación y actualización de stock)
  public async Task<Repositories.Entities.Product> FindProductByIdAsync(int productId) => await Products.FindAsync(productId);
  public void UpdateProduct(Repositories.Entities.Product product) => Update(product);

  // Agregar método para encontrar cliente por id (usado en validación de orden)
  public async Task<Repositories.Entities.Customer> FindCustomerByIdAsync(string customerId) => await Customers.FindAsync(customerId);

  // Validaciones para integridad referencial
